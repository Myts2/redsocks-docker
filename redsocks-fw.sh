...
fw_setup() {
  ...
  #iptables -t nat -A REDSOCKS -p udp --dport 53 -j REDIRECT --to-ports 5553
  #iptables -t nat -A REDSOCKS -p udp  --dport 53 -j DNAT --to-destination 127.0.0.1:5553
  iptables -t nat ! -d 114.114.114.114 -A REDSOCKS -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:5553
